...
app.use( "/assets" , express.static(path.join( __dirname , 'server/public/assets'))); // assigns a subdomain assets and stores files received in there to the directory received from dirname and further path ahead that is public/assets

//STORAGE
const storage = multer.diskStorage({
    destination :   function( req , file , cb){
        cb( null , "server/public/assets"); //The cb (callback) function is provided with two arguments: err and the path to the destination directory ("public/assets" in this case). If there's an error, you can pass the error to cb(err), otherwise, pass null.
    },
